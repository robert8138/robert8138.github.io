...
With so many data producers using our tools daily, workflow design mattered a lot. The number one concern from our users was always iteration speed. People wanted to create, test, and iterate on their changes with a fast feedback loop. That became our north star, and we kept improving validation, dry runs, and how changes got reviewed.

### Validation

Users declared semantic definitions in YAML, and not every configuration they wrote was correct, so the framework had to provide guardrails that surfaced misconfigurations as early as possible. We built a validation suite that ran at two points in the developer lifecycle, locally as users iterated on their configs, and in CI when a PR was created.
...
Owners were also the ones who could answer incoming questions about what a business definition actually meant. They understood the context better than the Minerva platform team did, so we surfaced them on every Minerva asset page in [Dataportal](https://medium.com/airbnb-engineering/democratizing-data-at-airbnb-852d76c51770), our data catalog. We did the same in SQL Lab, our SQL editor, so someone who hit a problem querying a metric or dimension could reach the owner directly. In the age of AI, it's not hard to imagine that these owners would become the curators and stewards of specific domain knowledge in their respective areas. This I imagine will be a key thing that drives context engineering at scale.
...
Building Minerva taught us that users care deeply about iteration speed, and that hasn't changed in the age of AI. If anything, the bar is higher now. We spent a lot of time on validation, catching errors as early as we could, and auto-fixing issues in the background when that made sense. The dry run workflow got heavy investment too, since it let people test changes end to end. Finally, we invest in ownership. We codified it across review and oncall, then put owners front and center in the data catalog, and that's what let us scale ownership in a distributed way.
